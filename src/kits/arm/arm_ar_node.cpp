...
int main(int argc, char ** argv) {
  ...
  while (ros::ok()) {
    ...
    if (diff.get(3) == experimental::MobileIODiff::ButtonState::ToOn) {
      ROS_WARN("AR Mode temporarily disabled, due to bug");
      /*
      ROS_INFO("Starting AR Mode");
    ...
    if (diff.get(6) == experimental::MobileIODiff::ButtonState::ToOn) {
      ROS_INFO("Starting Grav Comp Mode");
      arm -> cancelGoal();
      ar_mode = false;
    }
    ...
    if (diff.get(8) == experimental::MobileIODiff::ButtonState::ToOn) {
      ROS_INFO("AR Arm control node exit");
      // Clear MobileIO text
      mobile -> clearText();
      return 1;
    }
    ...
    if (ar_mode) {
      ...
      xyz_scale << 1, 1, 2;
      Eigen::Matrix3d rot_target = rot_phone_init.transpose() * rot_phone_target;
